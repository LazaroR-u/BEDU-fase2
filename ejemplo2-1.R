...
quantile(x, seq(0.1,0.9, by = 0.1)) # Deciles, hacemos una secuencia que va de 0.1 en 0.1
...
IQR(x) #45 es muy disperso, el rango intercuartilico es la diferencia 
#entre el penultimo cuantil y el primero.
quantile(x,probs=0.75 - quantile(x,probs= 0.25))
plot(x)
# o bien


#tercer cuantil menos el primero
quantile(x, probs = 0.75)  - quantile(x, probs = 0.25)

# La varianza y desviación estándar muestral en R las calculamos con las 
# siguientes instrucciones respectivamente

var(x) #varianza es igual a sd**2
sd(x)
sqrt(var(x)) #es igual que sd(x) 
(sd(x))**2 #es igual que var(x)
