...
setwd("C:/Users/Lazaro Diaz/Documents/BEDU/df/s2")
...
Scm <- merge(Sconf, Sdec) # Unimos infectados y decesos acumulados para cada fecha con merge
dim(Scm)

mex <- filter(Scm, Country == "Mexico") # Seleccionamos sólo a México
mex <- filter(mex, Infectados != 0) # Primer día de infectados

mex <- mutate(mex, NI = c(1, diff(Infectados))) # Nuevos infectados por día, diff(columna) devuelve la diferencia entre 
#el valor actual y el anterior de infectados.
...
mex <- mutate(mex, IDA = lag(Infectados), DDA = lag(Decesos)) # Valores día anterior, #lag(columna) devuelve el valor anterior
...
head(mex); tail(mex)
write.csv(mex, "C19Mexico.csv", row.names = FALSE) #guardamos el archivo csv 
